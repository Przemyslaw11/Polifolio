...
class PortfolioService:
    ...
    async def get_user_portfolio(
            ...
            try:
                ...
                price_data = await self.stock_service.get_latest_stock_prices(stock_symbols, db)
    ...
    async def _update_user_portfolio_history(self, user: User, db: AsyncSession):
        try:
            portfolio_items = await self.stock_service.get_user_portfolio_items(user.id, db)
            ...
            volatility = await self.stock_service.calculate_portfolio_volatility(portfolio_items)
